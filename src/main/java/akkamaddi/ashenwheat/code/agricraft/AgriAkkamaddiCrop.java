...
import akkamaddi.ashenwheat.code.AkkamaddiCrop;
...
abstract public class AgriAkkamaddiCrop implements ICropPlant 
{
	private static int[] GAIN_FORTUNE_MAPPING = { 0, 1, 1, 2, 2, 3, 4, 5, 7, 10 };
	...
	public ArrayList<ItemStack> getFruitsOnHarvest(int gain, Random rand) 
	{
		...
		while (n > 0) {
			...
			drops.add(new ItemStack(fruit.getItem(), rem));
			n -= rem;
		}
		return drops;
	}
    ...
    /**
     * This method is called when this crop is harvested, but before any default AgriCraft harvest 
     ...
     * By returning false from this method, you prevent Agricraft from doing any further 
     * harvesting operations, effectively cancelling the harvest,
    ...
    /**
     * This is called right before this plant is removed from crop sticks, or 
     * when the crop sticks are broken.
     *
     * @param world the World object
     * @param x the x coordinate
     * @param y the y coordinate
    ...
    /**
     * If you want your crop to have additional data, this is called when the plant is first 
     * applied to crop sticks, either trough planting, spreading or mutation
     *
     * @param world the world object for the crop
     * @param x the x coordinate
     * @param y the y coordinate
     ...
     */
	@Override
	public IAdditionalCropData getInitialCropData(World world, int x, int y,
			int z, ICrop crop) 
	{
		return null;
	}
    ...
    /**
     * If this CropPlant should track additional data, this method will be called when the 
     * crop containing such a CropPlant is reading from NBT
     *
     ...
     * @param crop the ICrop instance of the TileEntity (is the same object as the TileEntity, 
	...
	public IGrowthRequirement getGrowthRequirement() 
	{
		...
		return apiv2.createDefaultGrowthRequirement();
	}

    /**
     * This is called when a growth tick has been allowed. At this point the growth tick can 
     ...
     */
	@Override
	public boolean onAllowedGrowthTick(World world, int x, int y, int z,
			int oldGrowthStage) 
	{
		return true;
	}
	...
	public IIcon getPlantIcon(int growthStage) {
		return this.getBlock().getIcon(1, growthStage);
	}
    ...
    /** 
     * Return true if you want to render the plant yourself, else agricraft will 
     *  render the plant based on the data returned by the getIcon and renderAsFlower methods 
     */
	@Override
	...
	@Override
	@SideOnly(Side.CLIENT)
	public void renderPlantInCrop(IBlockAccess world, int x, int y, int z,
			RenderBlocks renderer) {
		// does nothing, should not ever be called.
	}
